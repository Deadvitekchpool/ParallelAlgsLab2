...
int main(int argc, char **argv)
{
    ...
    int rec_message; //message to receive
    //MPI_Sendrecv function simply sends message to the next process and receiving rec_message from previous process
    MPI_Sendrecv(&message, 1, MPI_INT, rank == size - 1 ? 0 : rank + 1, 0, 
    &rec_message, 1, MPI_INT, rank == 0 ? size - 1 : rank - 1, 0, MPI_COMM_WORLD, &status);
    printf("Process %d received message %d from process %d\n", rank, rec_message_one, status.MPI_SOURCE);
